...
โครงการนี้ออกแบบระบบในรูปแบบ **Layout-Aware Multi-Stage Pipeline** เพื่อแปลงเรซูเม่ที่อยู่ในรูปแบบภาพหรือไฟล์ PDF ซึ่งเป็นเอกสารที่ไม่มีโครงสร้าง ให้กลายเป็นข้อมูลเชิงโครงสร้างอย่างเป็นระบบ กระบวนการทำงานของระบบแบ่งออกเป็นหลายขั้นตอน โดยแต่ละขั้นมีหน้าที่ชัดเจนและเชื่อมต่อกันเป็นลำดับ
แนวคิดหลักของ Methodology คือการแยกการประมวลผลเชิงพื้นที่ของเอกสาร (spatial layout) ออกจากการประมวลผลเชิงความหมายของข้อความ (semantic content) เพื่อเพิ่มความแม่นยำและลดความกำกวมของข้อมูลในเรซูเม่ที่มีรูปแบบการจัดวางหลากหลาย

### 1. Layout Detection ด้วย YOLO11s
ขั้นตอนแรกของระบบคือการตรวจจับโครงสร้างของเรซูเม่จากภาพต้นฉบับ โดยใช้โมเดล **YOLO11s** ซึ่งเป็นโมเดลตรวจจับวัตถุแบบ single-stage ที่มีประสิทธิภาพสูง ระบบถูกฝึกให้สามารถระบุประเภทและขอบเขตของ section หลักภายในเรซูเม่ เช่น Personality, Education, Experience, Skills, Project และ Training

ผลลัพธ์จากขั้นตอนนี้คือชุดของ bounding box พร้อมป้ายกำกับของแต่ละ section ซึ่งแสดงตำแหน่งเชิงพื้นที่ของข้อมูลบนหน้าเอกสาร การตรวจจับ layout ก่อนการอ่านข้อความช่วยให้ระบบเข้าใจโครงสร้างของเรซูเม่ และเป็นพื้นฐานสำคัญสำหรับการประมวลผลในขั้นตอนถัดไป

### 2. การตัดภาพตาม Section (Section-based Image Cropping)
หลังจากได้ bounding box จาก YOLO11s ระบบจะทำการตัดภาพเรซูเม่ตามขอบเขตของแต่ละ section ขั้นตอนนี้ช่วยแยกส่วนข้อมูลออกจากกันอย่างชัดเจน และลดปัญหาการประมวลผลข้อมูลข้าม section ที่ไม่เกี่ยวข้องกัน
ภาพที่ได้จากขั้นตอนนี้จะถูกจัดเป็นชุดย่อยตามประเภทของ section และใช้เป็นข้อมูลนำเข้าสำหรับกระบวนการ OCR ต่อไป

### 3. Optical Character Recognition ด้วย Google Cloud Vision
ขั้นตอนถัดไปคือการแปลงข้อความจากภาพเป็นตัวอักษร โดยใช้ **Google Cloud Vision API** ซึ่งทำหน้าที่เป็น Optical Character Recognition (OCR) engine หลักของระบบ ระบบจะประมวลผลภาพที่ถูกตัดแยกตาม section เพื่อดึงข้อความออกมาเป็นลำดับตัวอักษร การทำ OCR แยกตาม section ช่วยลดสัญญาณรบกวนและรักษาบริบทของข้อความให้สอดคล้องกับโครงสร้างเอกสาร อีกทั้ง Google Cloud Vision ยังรองรับทั้งภาษาไทยและภาษาอังกฤษ ทำให้ระบบสามารถจัดการเรซูเม่ที่มีหลายภาษาได้อย่างมีประสิทธิภาพ

### 4. Semantic Information Extraction ด้วย NLP
เมื่อได้ข้อความจาก OCR แล้ว ระบบจะประมวลผลข้อความในขั้นตอน **Semantic Information Extraction** โดยใช้เทคนิค Natural Language Processing (NLP) เพื่อดึงข้อมูลเชิงความหมายออกจากข้อความในแต่ละ section การประมวลผลในขั้นตอนนี้ถูกออกแบบให้เป็น **Section-Aware** กล่าวคือ ข้อความจะถูกตีความภายใต้บริบทของ section ที่ปรากฏ
สำหรับข้อความภาษาไทย ระบบใช้ไลบรารี **PyThaiNLP** เพื่อช่วยในการตัดคำและจัดรูปแบบข้อความก่อนการดึงข้อมูลเชิงความหมาย เนื่องจากภาษาไทยไม่มีตัวแบ่งคำที่ชัดเจน ส่วนข้อความภาษาอังกฤษสามารถประมวลผลด้วยเทคนิค NLP ทั่วไปได้ การใช้ PyThaiNLP ช่วยเพิ่มความเหมาะสมของระบบในการจัดการเรซูเม่ภาษาไทยและลดความกำกวมของข้อมูล

### 5. การสร้างผลลัพธ์เชิงโครงสร้าง (Structured Output)
ข้อมูลที่ได้จากขั้นตอน Semantic Information Extraction จะถูกจัดระเบียบและแปลงให้อยู่ในรูปแบบข้อมูลเชิงโครงสร้าง เช่น JSON โดยโครงสร้างของข้อมูลสะท้อน section ต่าง ๆ ภายในเรซูเม่ ผลลัพธ์ในรูปแบบนี้ช่วยให้ข้อมูลสามารถนำไปใช้งานต่อในระบบอื่นได้อย่างสะดวก และรองรับการพัฒนาต่อยอดในอนาคต
